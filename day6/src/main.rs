...
fn parse_orbits(input: &str) -> Vec<Orbit> {
    input.lines().map(parse_orbit).collect()
}
...
fn parse_orbit(input: &str) -> Orbit {
    ...
    if parts.next().is_some() {
        println!("Something went wrong parsing orbit {}!", input);
    }
    Orbit {
...
mod tests {
    ...
    #[test]
    fn test_parse_orbit() {
        let orbit1 = parse_orbit("COM)A");
        let orbit2 = parse_orbit("A)B");
        let orbit3 = parse_orbit("A)C");

        assert_eq!(
            Orbit {
                center: "COM".to_string(),
                orbiter: "A".to_string()
            },
            orbit1
        );
